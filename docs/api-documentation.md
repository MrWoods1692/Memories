...
| 服务器 | 端口 | 跨域 (CORS) | 外网访问 | 局域网访问 | 用途 |
|--------|------|-------------|---------|-----------|------|
| **EmbeddedServer** (API) | `8080` | ✅ 允许任意来源 | ✅ 允许 | ✅ 允许 | 主 REST API 服务器 |
| **AdminServer** (静态文件) | `8081` | — | ❌ 禁止 | ✅ 允许 | 管理面板前端静态文件服务 |

两个服务器均基于 **NanoHTTPD** 构建。

- **API 服务器 (8080)**：允许外网和局域网访问，对所有响应自动添加 CORS 头（`Access-Control-Allow-Origin: *`），允许任意来源的前端页面跨域调用 API。
- **管理面板 (8081)**：仅提供静态文件服务，只接受局域网请求（IP 段：`10.x`、`172.16-31.x`、`192.168.x`、`127.x`），不对外暴露。
...
| 服务 | 端口 | 跨域 (CORS) | 外网访问 | 局域网访问 |
|------|------|------------|---------|-----------|
| API 服务器 | `8080` | ✅ 允许任意来源 | ✅ 允许 | ✅ 允许 |
| 管理面板 | `8081` | — | ❌ 禁止 | ✅ 允许 |
