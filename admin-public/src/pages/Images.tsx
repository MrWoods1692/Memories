...
export function ImagesPage({ toast }: Props) {
  ...
  const audit = useCallback(async (url: string, status: 1 | 2) => {
    setActing(p => new Set(p).add(url));
    try {
      await apiPost(`/images/audit?url=${encodeURIComponent(url)}&status=${status}`);
      toast(status === 1 ? '已通过' : '已拒绝');
      await load();
      setSelected(p => { const n = new Set(p); n.delete(url); return n; });
      return true;
    } catch { toast('操作失败', 'error'); return false; }
    finally { setActing(p => { const n = new Set(p); n.delete(url); return n; }); }
  }, [toast]);
  ...
  const batchAudit = async (status: 1 | 2) => {
    ...
    setConfirm({
      msg: `确定${status === 1 ? '通过' : '拒绝'}选中的 ${urls.length} 张图片？`,
      fn: async () => {
        let ok = 0;
        for (const url of urls) {
          const r = await audit(url, status);
          if (r) ok++;
        }
  ...
  const del = (url: string) => {
    setConfirm({
      msg: `确定删除图片？`,
      fn: async () => {
        try { await apiDelete(`/images/delete?url=${encodeURIComponent(url)}`); toast('已删除'); load(); }
  ...
  const selectAll = () => {
    ...
    else setSelected(new Set(pendings.map(i => i.url)));
  };
  ...
  const pendingIds = new Set(pendingList.map(i => i.url));
  ...
  const previewPrev = () => {
    for (let i = previewIdx - 1; i >= 0; i--) {
      if (pendingIds.has(filtered[i].url)) { setPreviewIdx(i); setZoomed(false); return; }
    }
  };
  ...
  const hasPrev = previewIdx > 0 && filtered.slice(0, previewIdx).some(i => pendingIds.has(i.url));
  const hasNext = previewIdx < filtered.length - 1 && filtered.slice(previewIdx + 1).some(i => pendingIds.has(i.url));

  const handlePreviewAudit = async (status: 1 | 2) => {
    if (!previewItem) return;
    const ok = await audit(previewItem.url, status);
        ...
        {loading ? <div className="skeleton" style={{height:200}} /> : filtered.length === 0 ? (
          ...
          <div className="table-wrap">
            <table>
              ...
              <tbody>
                {filtered.map((i, idx) => (
                  <tr key={i.url} className={selected.has(i.url) ? 'row-selected' : ''}>
                    <td style={{textAlign:'center'}}>
                      {i.status === 0 && (
                        <input type="checkbox" checked={selected.has(i.url)}
                          onChange={() => toggleSelect(i.url)}
                          style={{width:14,height:14,margin:0,cursor:'pointer'}} />
                      )}
                    </td>
                    <td style={{color:'var(--accent)',fontWeight:600}}>#{i.url}</td>
                    <td>
                      <div className="thumb-cell" onClick={() => goPreview(idx)} title="点击预览">
                        <img src={i.url} alt={`#${i.url}`} loading="lazy"
                    ...
                    <td>
                      <div className="btn-row">
                        {i.status === 0 && (
                          <>
                            <button className="btn success xs" disabled={acting.has(i.url)} onClick={() => audit(i.url, 1)}><IconCheck size={12} /> 通过</button>
                            <button className="btn warn xs" disabled={acting.has(i.url)} onClick={() => audit(i.url, 2)}><IconX size={12} /> 拒绝</button>
                          </>
                        )}
                        {i.status !== 0 && isAdmin && (
                          <button className="btn danger xs" onClick={() => del(i.url)}><IconTrash size={12} /> 删除</button>
                        )}
                      </div>
                    </td>
                  </tr>
          ...
          <div className="gallery-grid">
            {filtered.map((i, idx) => (
              <div key={i.url}
                className={`gallery-item ${selected.has(i.url) ? 'selected' : ''}`}
                onClick={() => goPreview(idx)}
                onContextMenu={e => { e.preventDefault(); if (i.status === 0) toggleSelect(i.url); }}
              >
                <span className="gallery-id">#{i.url}</span>
                <span className={`gallery-badge gallery-status-${i.status}`}>{s[i.status]}</span>
                {i.status === 0 && (
                  <div className="gallery-check" onClick={e => { e.stopPropagation(); toggleSelect(i.url); }}>
                    {selected.has(i.url) && <IconCheck size={14} />}
                  </div>
                )}
                <img src={i.url} alt={`#${i.url}`} loading="lazy"
                  onError={e => {
          ...
          <div className="preview-dialog" onClick={e => e.stopPropagation()}>
            ...
            <div className="preview-body">
              <div className="preview-body-inner">
                <img src={previewItem.url} alt={`#${previewItem.url}`}
            ...
            <div className="preview-actions">
              <div className="preview-nav">
                ...
                <span className="preview-pos">
                  {pendingList.findIndex(i => i.url === previewItem.url) + 1} / {pendingList.length} 待审
                </span>
              ...
              <div className="preview-audit">
                {previewItem.status === 0 ? (
                  <>
                    <button className="btn success" disabled={acting.has(previewItem.url)} onClick={() => handlePreviewAudit(1)} title="通过 (A)">
                      <IconCheck size={16} /> 通过
                    </button>
                    <button className="btn danger" disabled={acting.has(previewItem.url)} onClick={() => handlePreviewAudit(2)} title="拒绝 (R)">
                      <IconX size={16} /> 不通过
                    </button>
                  </>
                ) : isAdmin ? (
                  <button className="btn danger" onClick={() => { del(previewItem.url); setPreviewIdx(-1); }}>
